# -*- coding: utf-8 -*-
"""
Created on Tue Oct 20 16:40:33 2020
...
class gomoku_ai:
    def __init__(self, clr):
        ...
        self.__data_set1 = [[0 for i in range(19)] for j in range(19)]
        self.__data_set2 = [[0 for i in range(19)] for j in range(19)]
        
        self.__dx = [0,1,1,1,0,-1,-1,-1]
        self.__dy = [-1,-1,0,1,1,1,0,-1]
        self.__dx1 = [0,1,2,2,2,2,2,1,0,-1,-2,-2,-2,-2,-2,-1]
        self.__dy1 = [-2,-2,-2,-1,0,1,2,2,2,2,2,-1,0,-1,-2,-2]
        self.__b_len = 19
        self.__score_four = 450
        self.__score_four_2 = 450
        self.__score_three= 400
        self.__score_three_2= 370
        self.__f_d = 150
        self.__t_d =150
        self.__two_d = 5
        self.__date_wight = 30
    ...
    def next_stone(self, board, length):
        if self._first == 0 and self._color == -1: #검은돌이 처음일 때 오목판의 중앙에 돌을 놓고 날일진 좌표 계산
            stn = stone(self._color, length)
            stn.setX(9)
            stn.setY(9)
            self.create_data_Set1(9, 9, self.__data_set1)
            self.create_data_Set2(9, 9, self.__data_set2)
            self._first = 1
            return stn
        ...
        if self._first == 1:  #두 번 째 차례일 때 계산한 두개의 날일진 좌표에 가중치를 더해줌
            self._first = 2
            for i in able_stn:
                board_weight1[i[0]][i[1]] += (self.__data_set1[i[0]][i[1]]* 50)
                board_weight1[i[0]][i[1]] += (self.__data_set2[i[0]][i[1]]* 50)
        elif self._first == 2:  #3번째 좌표일 때 어느 날일진에 해당하는지 계산함
            self._first = 3     #만약 날일진에 해당하지 않는다면 날일진을 사용하지 않음
            
            if self.match_dataset(board, self.__data_set1, 2, self._color):
                self.__data_set = self.__data_set1
            elif self.match_dataset(board, self.__data_set2, 2, self._color):
        ...
        for i in able_stn:  #공격에 해당되는 가중치를 구함 자신이 공격 중 일 경우 자신의 공격의 가중치가 더 높고
            if self.__attack == 1: #방어 중 일 경우 상대의 공격 가중치가 더 높아짐
                ...
                self.Calc_Attack(board, board_weight1, self._color, i, 1)
        
        
        #self.print_b(board_weight1)
        #print()
        #self.print_b(board_weight2)
        
        temp3 = copy.deepcopy(board_weight1) 
        for i in able_stn: #더 유리한 수를 찾기 위해 각각의 가중치를 1/2씩 더함
            board_weight1[i[0]][i[1]] += (board_weight2[i[0]][i[1]]//2) # + (self.__data_set[able_stn[i][0]][able_stn[i][1]] * 50))
            board_weight2[i[0]][i[1]] += ((temp3[i[0]][i[1]]//2) + (self.__data_set[i[0]][i[1]] * self.__date_wight))
        del temp3
        ...
        if temp[0][2] < temp[1][2] + 130 and temp[0][2] > 3000:   #자신 또는 상대가 공격중일 경우 더욱 유리한 곳으로 공격 또는 방어를 진행
        ...
        if self._first == 0 and self._color == 1: #흰돌이 첫 수 일 때 놓여진 좌표로 날일진 좌표를 구함
            self._first = 1
            self.create_data_Set1(temp[0][0], temp[0][1], self.__data_set1)
            self.create_data_Set2(temp[0][0], temp[0][1], self.__data_set2)

        return stn  
    def print_b(self, board):
        for i  in range(0,19):
            print("{0:^5}".format(i), end="") # no newline
        print()
        for i  in range(19-1,-1, -1):
                for j  in range(0, 19):
                   val=board[i][j] # no newline
                   print("{0:^5}".format(val), end="") # no newline
                print()
    ...
    def Calc_Attack(self, board, board_weight, clr, p, t):
        ...
        # -1: 상대가 놓은돌
        # 0 : 아무도 놓지 않은 경우
        #반복문을 통해 4방향 혹은 8방향에 대해서 가중치 검사를 진행
    ...
    def create_data_Set1(self,x,y, board): #날일진을 만드는 메소드 첫 번째
        if board[x][y] == 1: #재귀함수로 구현함
            return
        
        board[x][y] = 1
    
        if x-2 >= 0 and y-1 >= 0 :
            self.create_data_Set1(x-2, y-1,board)
        if x-1 >= 0 and y+2 < 19 :    
            self.create_data_Set1(x-1, y+2, board)
        if x+2 < 19 and y+1 < 19 :
            self.create_data_Set1(x+2, y+1, board)
        if x+1 < 19 and y-2 >= 0 :
            self.create_data_Set1(x+1, y-2, board)
        
    def create_data_Set2(self, x,y,board): #날일진을 만드는 메소드 두 번째
        if board[x][y] == 1:
            return
        ...
        if x-2 >= 0 and y+1 < 19 :
            self.create_data_Set2(x-2, y+1, board)
        if x-1 >= 0 and y-2 >= 0 :    
            self.create_data_Set2(x-1, y-2, board)
        if x+2 < 19 and y-1 >= 0 :
            self.create_data_Set2(x+2, y-1, board)
        if x+1 < 19 and y+2 < 19 :
            self.create_data_Set2(x+1, y+2, board)
    
    def match_dataset(self, board,dataset,cnt, clr):
        flag = 0 #날일진에 해당되는 좌표를 검사하기위한 메소드
        for i in range(len(board)):
            for j in range(len(board)):
                if board[i][j] == clr and dataset[i][j] == 1:
                    flag += 1
                elif board[i][j] == clr*-1:
                    return False
